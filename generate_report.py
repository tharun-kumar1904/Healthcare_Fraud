...
Uses a clean corporate black-and-white style.
"""
import os
import json
import csv
...
# Set colors (Clean Black-and-White Corporate Style)
COLOR_PRIMARY = RGBColor(0x00, 0x00, 0x00) # Pure Black
...
def set_table_borders(table, color="555555"):
...
def create_table(headers, data, widths=None):
    ...
    # Format header row (White / Light Gray Background, Black Text)
...
# ==========================================
# 1. COVER PAGE (B&W Corporate Style)
# ==========================================
p_cover_space = doc.add_paragraph()
p_cover_space.paragraph_format.space_before = Pt(140)
...
sub_run = sub_p.add_run("A Machine Learning Stacking Ensemble Approach for Identifying Anomalous Provider Billing Signatures in Medicare Claims")
...
# ==========================================
# 3. KEY RESULTS (VISUAL KPI PAGE)
# ==========================================
add_heading("2. Final Model Performance", level=1)
add_paragraph(
    "The stacking ensemble model was evaluated on an unseen out-of-sample holdout dataset (representing 10% of total providers) to verify generalization. "
    "Below are the primary performance metrics achieved at the F1-optimal decision threshold (0.85):"
)

# Create a clean 2x2 grid for KPIs with borders and bold formatting
kpi_table = doc.add_table(rows=2, cols=2)
kpi_table.alignment = WD_TABLE_ALIGNMENT.CENTER
set_table_borders(kpi_table, COLOR_BORDER)

kpi_data = [
    [("ROC-AUC", "0.9579"), ("Recall (Sensitivity)", "74.5%")],
    [("Precision", "56.7%"), ("F1-Score", "0.644")]
]

for row_idx, row_content in enumerate(kpi_data):
    hdr_cells = kpi_table.rows[row_idx].cells
    for col_idx, (label, val) in enumerate(row_content):
        set_cell_background(hdr_cells[col_idx], "F9FBFD" if row_idx == col_idx else "FFFFFF")
        set_cell_margins(hdr_cells[col_idx], top=200, bottom=200, left=250, right=250)
...
findings_headers = ["Finding Indicator", "Observation Summary", "Operational Multiplier"]
findings_data = [
...
# 5. PROBLEM STATEMENT
# ==========================================
add_heading("4. Problem Statement & Fraud Scenarios", level=1)
add_paragraph(
    "Healthcare fraud, waste, and abuse (FWA) represents a threat to the financial stability of insurance providers and significantly inflates patient premiums. "
    ...
    "Traditional auditing mechanisms depend on administrative reviews or rules-based software that flags simple claims-level anomalies. "
...
add_paragraph("Billing for more expensive diagnostic groups or complex procedures than what was clinically indicated or performed (often flagged by inflated diagnosis code counts).", bold_prefix="• Diagnostic Upcoding: ")
add_paragraph("Attending and operating physicians colluding to bill multiple claims under a single, patient ID (re-coding single claims across multiple physician IDs).", bold_prefix="• Referral and Physician Rings: ")
add_paragraph("Submitting aesthetic, cosmetic, or elective procedures under medically necessary diagnostic groups to force insurance reimbursements.", bold_prefix="• Non-covered Services: ")
...
# 6. DATASET OVERVIEW
# ==========================================
add_heading("5. Dataset Overview & Relational Architecture", level=1)
add_paragraph(
...
# 11. MODEL EVALUATION
# ==========================================
add_heading("10. Model Evaluation & Threshold Optimization", level=1)
add_paragraph(
    ...
    "The table below details the performance metrics of the evaluated models in a clean business format:"
)
...
# ==========================================
# 12. CHALLENGES & LIMITATIONS (NEW SECTION)
# ==========================================
add_heading("11. Challenges & Limitations", level=1)
add_paragraph(
    "Developing a reliable provider-level fraud detection framework presents several real-world engineering challenges and constraints that must be accounted for during deployment."
)

add_heading("Challenges Faced", level=2)
add_paragraph("Fraud targets represent only 9.7% of the historical provider dataset. Standard estimators tend to over-predict the majority class. Preserving the physical boundaries of engineered metrics meant configuring class weights natively during model training rather than utilizing SMOTE-based synthetic methods.", bold_prefix="• Highly Imbalanced Fraud Labels: ")
add_paragraph("Translating patient-level claims to provider-level features requires massive aggregation runs. Calculating metrics like standard deviations and unique beneficiary ratios across 558K claims is computationally expensive and requires optimized database indices.", bold_prefix="• High Aggregation Overhead: ")
add_paragraph("Key claims columns (such as operating physician ID, other physician ID, and outpatient discharge dates) contain substantial missing records, requiring strict placeholder imputation schemes to prevent row loss during multi-table joins.", bold_prefix="• Missing Clinical & Administrative Data: ")
add_paragraph("In a fraud screening context, any threshold adjustment introduces a direct trade-off: higher precision reduces false positives but misses fraud cases, while higher recall flags more cases but increases investigator auditing backlogs.", bold_prefix="• Threshold Selection Trade-offs: ")
add_paragraph("The target label represents 'Potential Fraud' rather than legally adjudicated fraud. This introduces a slight level of noise in the training target vector, meaning the model is predicting anomalous billing behaviors rather than confirmed legal guilt.", bold_prefix="• Label Uncertainty: ")

add_heading("System Limitations", level=2)
add_paragraph("The stacking ensemble operates under the assumption that future billing fraud will mirror historical billing behaviors. The model cannot predict entirely novel or clean billing techniques that have never been seen in past claims data.", bold_prefix="• Reliance on Historical Patterns: ")
add_paragraph("Changes in medical coding standards, insurance deductibles, or policy restrictions will introduce feature drift, requiring regular runs of the pipeline to maintain score accuracy.", bold_prefix="• Model Drift & Policy Shifts: ")
...
# 14. BUSINESS IMPACT
# ==========================================
add_heading("13. Business Impact & ROI Analysis", level=1)
add_paragraph(
...
# ==========================================
# 15. PRACTICAL DEPLOYMENT STRATEGY (NEW SECTION)
# ==========================================
...
if os.path.exists(overview_img):
    ...
    r_cap = p_cap.add_run("Figure 1: Overview Dashboard")
    r_cap.italic = True
...
if os.path.exists(fraud_patterns_img):
    ...
    p_cap = doc.add_paragraph()
    p_cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r_cap = p_cap.add_run("Figure 2: Fraud Patterns Dashboard")
    r_cap.italic = True
    r_cap.font.size = Pt(9.5)
    r_cap.font.color.rgb = COLOR_PRIMARY
doc.add_paragraph().paragraph_format.space_after = Pt(12)
...
if os.path.exists(data_analysis_img):
    ...
    p_cap = doc.add_paragraph()
    p_cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r_cap = p_cap.add_run("Figure 3: Data Analysis Dashboard")
    r_cap.italic = True
    r_cap.font.size = Pt(9.5)
    r_cap.font.color.rgb = COLOR_PRIMARY
doc.add_paragraph().paragraph_format.space_after = Pt(12)
...
if os.path.exists(model_evaluation_img):
    ...
    p_cap = doc.add_paragraph()
    p_cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r_cap = p_cap.add_run("Figure 4: Model Evaluation Dashboard")
    r_cap.italic = True
    r_cap.font.size = Pt(9.5)
    r_cap.font.color.rgb = COLOR_PRIMARY
doc.add_paragraph().paragraph_format.space_after = Pt(12)
...
if os.path.exists(predictions_img):
    ...
    p_cap = doc.add_paragraph()
    p_cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r_cap = p_cap.add_run("Figure 5: Predictions Dashboard")
    r_cap.italic = True
    r_cap.font.size = Pt(9.5)
    r_cap.font.color.rgb = COLOR_PRIMARY
doc.add_paragraph().paragraph_format.space_after = Pt(12)
...
if os.path.exists(business_impact_img):
    ...
    p_cap = doc.add_paragraph()
    p_cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r_cap = p_cap.add_run("Figure 6: Business Impact Dashboard")
    r_cap.italic = True
    r_cap.font.size = Pt(9.5)
    r_cap.font.color.rgb = COLOR_PRIMARY
doc.add_paragraph().paragraph_format.space_after = Pt(12)
...
# 18. APPENDIX
# ==========================================
add_heading("17. Appendix", level=1)
...
tech_headers = ["Technology / Library", "Role in Project Pipeline"]
tech_data = [
    ["Python", "Execution environment and pipeline runtime"],
    ["Pandas", "Data manipulation, alignment, and aggregation"],
    ["NumPy", "Vectorized calculations, polyfit regression curves"],
    ["Scikit-Learn", "Cross-validation splitting, metric scoring, Logistic Regression meta-classifier"],
    ["XGBoost", "Gradient boosted base model (Optuna-tuned)"],
    ["LightGBM", "DART base model classifier"],
    ["CatBoost", "Categorical-optimized gradient boosting classifier"],
    ["Streamlit", "Dashboard deployment, UI components, ROI calculator"],
    ["Plotly", "Interactive visualizations, box plots, ROI curves"],
    ["python-docx", "Word Report compiler and style formatting"]
]
create_table(tech_headers, tech_data, widths=[Inches(2.5), Inches(4.5)])
